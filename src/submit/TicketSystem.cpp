...
int main() {
    ...
    while (getline(std::cin, line)) {
        ...
        try {
            if (command == "add_user") {
                std::cout << userManager.addUser(
                    params['c'], params['u'], params['p'],
                    params['n'], params['m'], std::stoi(params['g'])
                );
            }
            else if (command == "login") {
                std::cout << userManager.login(params['u'], params['p']);
            }
            else if (command == "logout") {
                std::cout << userManager.logout(params['u']);
            }
            else if (command == "query_profile") {
                std::string result = userManager.queryProfile(params['c'], params['u']);
                std::cout << (result.empty() ? "-1" : result);
            }
            else if (command == "modify_profile") {
                std::string result = userManager.modifyProfile(
                    params['c'], params['u'],
                    params.count('p') ? params['p'] : "",
                ...
                std::cout << trainManager.addTrain(
                    ...
                    params['s'], params['p'], params['x'],
                    params['t'], params['o'], params['d'], params['y']
                );
            }
                ...
                std::string result = trainManager.queryTicket(
                ...
                std::string result = orderManager.buyTicket(
                    params['u'], params['i'], params['d'],
                    std::stoi(params['n']), params['f'], params['t'], queue
                );
                std::cout << result;
            }
